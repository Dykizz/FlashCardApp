...
function shuffleOptionsAndTrack(q: Question): Question {
  const originalOptions = q.options;
  const oldCorrectAnswerIndex = q.correctAnswer;
  const length = originalOptions.length;
  ...
  const shuffledOptions = indices.map((oldIndex) => originalOptions[oldIndex]);

  const newCorrectAnswerIndex = indices.indexOf(oldCorrectAnswerIndex);
...
export function usePriorityQueue(
  ...
  const currentQuestion = useMemo(() => {
    ...
    if (!isInfiniteLoop) {
      return shuffleOptionsAndTrack(frozenQueue[currentIndex]) || null;
    }
    return shuffleOptionsAndTrack(frozenQueue[0]) || null;
